...
public class Battle : GridBehaviour<FlatHexPoint>
{
	...
	private void CreateUnit (FlatHexPoint _point, Unit _unit, Player _owner)
	{

		Unit unit = Instantiate (_unit, Sector.Map [_point], Quaternion.identity) as Unit;

		Sector.Grid [_point].contents = Cell.Contents.unit;
		Sector.Grid [_point].unit = unit;
		Sector.Grid  [_point].unit.owner = _owner;
